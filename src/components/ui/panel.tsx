...
export function Panel({ title, onClose, children }: PanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      >
        {/* Header */}
        <div className={`flex items-center justify-between border-b ${colors.borderPrimary} flex-shrink-0`}>
          <div className="px-4 py-3">
            <span className={typography.mono.label}>{title}</span>
          </div>
          <button
            onClick={onClose}
            className={`px-4 py-3 ${typography.mono.caption} hover:opacity-60 ${interactive.transition} ${interactive.cursorPointer}`}
            aria-label="Close"
          >
            &times;
          </button>
